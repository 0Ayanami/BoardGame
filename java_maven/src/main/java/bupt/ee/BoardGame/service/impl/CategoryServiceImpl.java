...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Category> findAll() {
        ...
        //redis缓存优化
        //从redis中查询
        //获取jedis客户端（java实现redis的一个包）
        Jedis jedis = JedisUtil.getJedis();//参数为空默认访问本机的redis
        //查询--使用sortedset排序查询，这样以后的缓存数据就是有序的
        //Set<String> categorys = jedis.zrange("category", 0, -1);//获取该键对应的所有值

        //查询sortedset中的分数(cid)和值(cname)
        Set<Tuple> categorys = jedis.zrangeWithScores("category", 0, -1);

        List<Category> cs = null;
        //判断查询的集合是否为空:categorys
        //如果为空，需要从数据库查询，再将数据存入redis
        if (categorys == null || categorys.size() == 0){
            //System.out.println("从数据库查询数据。。。");
            //3.1redis缓存为空，从数据库中查询，并将数据存储进缓存
            cs = categoryDao.findAll();//返回一个list类型的结果集，该方法返回list类型，不需要做类型转换
            // 注意，直接从数据库查询返回的数据是表中数据排列的顺序

            //3.2将集合数据存储进redis缓存中，以后可以直接返回redis中的数据，存储的redis格式为sortedset类型
            ...
            //System.out.println("从redis缓存查询数据。。。");
